...
Метод `LaravelResponse::generate` принимает ассоциативный массив из команд и их параметров, которые будут выполнены на клиенской машине. Изначально библиотека поддерживает лишь две команды: `validation_errors` и `redirect`. Первая команда используется библиотекой для отправки ошибок формы, вторая - для редиректа пользователя в другое место. Вы можете легко расширить функционал библиотеки, добавив свои собственные обработчики команд, посмотрев на примеры того, как это сделано в [оригинальном коде](https://github.com/believer-ufa/prettyforms/blob/master/prettyforms.js#L563)
